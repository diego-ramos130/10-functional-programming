...
(function(module){
  ...
  Article.numWordsAll = () => {
    return app.Article.all.map(article => article.body.split(' ').length).reduce((a, i) => a + i);
  };

  Article.allAuthors = () => {
    return app.Article.all.map(article => article.author).sort().reduce((a,i) => {
      let length = a.length;
      if (length === 0 || a[length-1] !== i) {
        a.push(i)
      }
      return a;
    }, []);
  ...
  Article.numWordsByAuthor = () => {
    return app.Article.allAuthors().map(author => {
      let object = {};
      object.auth = author;
      object.numWords = app.Article.all.filter(article => article.author === author).map(article => article.body.split(' ').length).reduce((a, i) => a + i);
      return object;
    })
  };
